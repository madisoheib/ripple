# Benchmark — Ripple vs Laravel Reverb (Linux)
...
**Setups.** Ripple: the release binary, stock config. Reverb: Laravel 13 +
...
| Connections | Ripple RSS | Reverb RSS |
|---|---|---|
| 10,000 | 131 MiB | — |
| 20,000 | 258 MiB | — |
...
Both idle at 0% CPU. Ripple memory is linear from 1k to 60k. The spec target
...
| | delivered | p50 | p99 |
|---|---|---|---|
| Ripple | 10,000/10,000 | **94 ms** | 139 ms |
| Reverb | 10,000/10,000 | 122 ms | 164 ms |

**Server-side vs end-to-end.** Ripple's `/metrics` exposes the
...
|---|---|---|---|---|
| Ripple | 100% | **14.7 ms** | **32 ms** | 27% / 33% *of 200% (2 cores)* |
...
load: Reverb can grow ~1.6× before saturating; Ripple ~7× — and it can use
additional cores, which Reverb structurally cannot.

## 4. Operational notes

- Ripple's accept backlog was 128 (std default) before this benchmark; ramp
...
systemd-run --unit=ripple -p LimitNOFILE=1000000 -p CPUAffinity="0 1" \
  ripple start --config /etc/ripple.toml
# generator (Node ≥16, npm i pusher ws):
BENCH_CONNS=40000 BENCH_HOLD=20000 taskset -c 2,3 node bench.mjs ripple idle
BENCH_CONNS=10000 taskset -c 2,3 node bench.mjs ripple fanout
BENCH_CONNS=2000 BENCH_RATE=25 BENCH_SECONDS=15 taskset -c 2,3 node bench.mjs ripple sustained
```
